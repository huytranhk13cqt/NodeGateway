...
const middleware = {
	...
	checkLoginBefore: async (req, res, next) => {
		try {
			...
			// valid = userNameDB tức là tìm được user này rồi
			// valid = 0 nếu sai các điều kiện ràng buộc
			const valid = middleware.validUserName(userNameBody, passWordBody);

			if (valid) {
				const user = valid; // userName trong DB
	...
	checkLoginStatus: async (req, res, next) => {
		try {
			...
			if (tokenHeader) {
				const object = middleware.parseJwt(tokenHeader);
